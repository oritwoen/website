  ...
  >
    <template #left>
      <div class="flex items-center gap-2 font-mono text-xs text-dimmed flex-wrap">
        ...
        <span class="flex items-center gap-1" :title="`ascii → hex: ${'oritwoen'} = ${blockHex}`">
          <UIcon name="i-simple-icons-bitcoin" class="size-3 text-[#F7931A]" />
          <span class="tabular-nums">block #{{ blockHex }}</span>
        </span>
        <span class="text-neutral mx-1">│</span>
        <span class="flex items-center gap-1" :title="`ascii → hex: oritwoen×4 = ${txHex}`">
          <UIcon name="i-simple-icons-ethereum" class="size-3 text-[#627EEA]" />
